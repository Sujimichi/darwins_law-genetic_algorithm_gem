...
module MatingRituals
  ...
  def microbial_recombination    
    ...
    index = select_index(2)
    pair = select_sorted_pair(index)
    #Replace % of weaker member's genes with fitter member's with a posibility of mutation.
    @population[index.last] = recombine *pair
    show_breeding_event pair.first, pair.last, @population[index.last] if @verbose && @verbose[:breeding_details]
  end
