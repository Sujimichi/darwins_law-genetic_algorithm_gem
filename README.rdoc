...
You can initialize the Standard or Microbial GA from the class <tt>GA</tt> or <tt>GeneticAlgorithm</tt>.  Both GAs inherit the same Base class so they are used in the same way and are interchangeable.  The args are given as a hash and the same args can be supplied to either GA.
