...
DarwinsLaw is a Ruby gem which provides an easy to use Genetic Algorithm(GA) which can be extended with a number of modules to produce different outputs and alter GA performance.


  ga = GA::Standard.new(:gene_length => 10, :mutation_rate => 0.2, :fitness => Proc.new{|genome|
    #add code to evaluate genome and return fitness score
  })
  ...
  ga.evolve                 #GA will run faster, re-encountered genomes are not re-tested


Additional arguments (see later) can be supplied to control how evolution runs and further modules can be added to change specific aspects of the GA (see later).

The *fitness* *function* is given as a Proc which you define in the initialization of the GA, it determines how effective a genome is and forms the basis for selection.  It is specific to your chosen application, so I'll leave that up to you!  See more about writing the fitness function later.


In addition to selection the other two core aspects to evolution are recombination and mutation.  As well as setting a rate for mutation you can also define the mutation function, also as a Proc or select from two predefined functions by passing the symbols :decimal or :binary (see more later).  The use of recombination and selection go hand in hand and there are two types of genetic algorithm (GA) which do slightly different styles; <tt>GA::Standard</tt> and <tt>GA::Microbial</tt>.

The Standard GA selects two members (parents) from the population at random and produces an offspring by recombination of the parents' genomes, by default 50% of the offspring’s genes comes from either parent.  A third member is selected at random as a contestant for the offspring and on;y the 'fittest' one will be allowed back to the population.


The Microbial style selects two members at random and determines which is fitter.  It then overwrites a % (70% by default) of the weaker members genome with genes from the fitter one.
...
Then initialize the Standard or Microbial GA from the class <tt>GA</tt> or <tt>GeneticAlgorithm</tt>.

  ga = GeneticAlgorithm::Standard.new(args)   # or GA::Standard.new(args)
  ga = GeneticAlgorithm::Microbial.new(args)  # or GA::Microbial.new(args)

Both GAs inherit the same Base class so they are used in the same way and are interchangeable.  The args are given as a hash and the same args can be supplied to either GA.

  ga = GeneticAlgorithm::Standard.new(:gene_length => 10, :popsize => 30, :mutation_rate => 0.1,
...
* <tt>:generations => 500     #Integer  </tt> - sets how many cycles (how long) the GA will run.
...
* <tt>:cross_over_rate => 0.5 #Float 0.0 to 1.0</tt> - controls the share of parent genomes used in producing an offspring.  0.5 means 50% from either parent which is ideal for Standard evolution.  When using Microbial evolution a cross over rate of 0.7 is more ideal.  Either will run with any > 0 cross over rate, the right rate depends on complexity of evolutionary search space.
...
* <tt>:mutation_function => :rand # Symbol or Proc</tt> - defines how a gene will be mutated.  When given the symbol :rand a gene will be mutated by a small +/- value, with :binary it will mutate 0->1 and 1->0.  It also takes a Proc which will be passed the gene to be mutated and should return the mutated value.
...
In addition to the above args which effect evolution there are additional args which effect GA output and extend GA functionality.
